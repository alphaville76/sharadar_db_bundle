...
class TBillBeta(CustomFactor):
    ...
    def compute(self, today, assets, out, close, rate):
        ...
        t_R = np.exp(t_r) - 1.0
        # The T-Bill rate is annually, therefore we need to make it monthly
        t_monthly_R = (1.0 + t_R)**(1.0 / 12.0) - 1.0

        beta = beta_residual(monthly_R, t_monthly_R, standardize=True)[0]
        out[:] = beta
...
class TBillBondSpreadBeta(CustomFactor):
    ...
    def compute(self, today, assets, out, close):
        ...
        spreads = t_bond_30y - t_bill_3m
        t_r = np.log1p(spreads)
        t_r = np.mean(t_r.reshape(-1, 21), axis=1)[1:].reshape(11, 1)
        t_R = np.exp(t_r) - 1.0
        # The T-Bill rate is annually, therefore we need to make it monthly
        t_monthly_R = (1.0 + t_R) ** (1.0 / 12.0) - 1.0
